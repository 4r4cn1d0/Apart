    ...
    def run_training(self, training_args: Dict[str, Any], run_in_background: bool = True) -> None:
        ...
        for key, value in training_args.items():
            if value is not None:
                ...
                python_args.append(f"--{arg_name} {value}")
        
        python_cmd = "python train_ppo.py " + " ".join(python_args)
        train_cmd = " && ".join(["cd ~/game/rl_env", "source ~/game/venv/bin/activate", python_cmd])
