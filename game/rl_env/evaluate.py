...
def evaluate_model(
    ...
    **env_kwargs
    ...
    # Phase 5: Enhanced metrics tracking
    episode_crops_harvested = []
    episode_shop_interactions = []
    episode_tree_interactions = []
    episode_farming_cycles = []
    episode_crop_diversity = []  # Ratio of types harvested
    episode_corn_harvested = []
    episode_tomato_harvested = []
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            for event in events:
                if event == 'harvest':
                    ...
                    tomato_count += 1
                elif event in ['buy_seed', 'sell_item']:
                    shop_interactions += 1
                elif event in ['apple_collect', 'wood_collect']:
                    tree_interactions += 1
        ...
        # Get farming cycles from env
        unwrapped_env = env.unwrapped
        farming_cycles = getattr(unwrapped_env, 'farming_cycles_completed', 0)
        
        # Calculate crop diversity (0-1 scale based on balance)
        total_crops = corn_count + tomato_count
        if total_crops > 0:
            diversity = min(corn_count, tomato_count) / max(corn_count, tomato_count) if max(corn_count, tomato_count) > 0 else 0
        else:
            diversity = 0.0
        ...
        # Phase 5: Store enhanced metrics
        episode_crops_harvested.append(crops_harvested)
        episode_shop_interactions.append(shop_interactions)
        episode_tree_interactions.append(tree_interactions)
        episode_farming_cycles.append(farming_cycles)
        episode_crop_diversity.append(diversity)
        episode_corn_harvested.append(corn_count)
        episode_tomato_harvested.append(tomato_count)
        ...
        print(f"Episode {episode + 1}/{n_episodes}: "
              ...
              f"Days: {final_day}, "
              f"Crops: {crops_harvested} (C:{corn_count}/T:{tomato_count})")
    ...
    print("-"*60)
    print("FARMING METRICS:")
    print(f"  Mean Crops Harvested: {np.mean(episode_crops_harvested):.1f} ± {np.std(episode_crops_harvested):.1f}")
    print(f"  Mean Corn Harvested: {np.mean(episode_corn_harvested):.1f}")
    print(f"  Mean Tomato Harvested: {np.mean(episode_tomato_harvested):.1f}")
    print(f"  Mean Farming Cycles: {np.mean(episode_farming_cycles):.1f}")
    print(f"  Mean Crop Diversity: {np.mean(episode_crop_diversity):.2f} (0=single type, 1=balanced)")
    print("-"*60)
    print("INTERACTION METRICS:")
    print(f"  Mean Shop Interactions: {np.mean(episode_shop_interactions):.1f}")
    print(f"  Mean Tree Interactions: {np.mean(episode_tree_interactions):.1f}")
    print("-"*60)
    print("MAXIMUMS:")
    print(f"  Max Reward: {np.max(episode_rewards):.2f}")
    print(f"  Max Money: ${np.max(episode_money):.2f}")
    print(f"  Max Crops: {np.max(episode_crops_harvested)}")
    print("="*60)
    ...
    return {
        ...
        'crops_harvested': episode_crops_harvested,
        'shop_interactions': episode_shop_interactions,
        'tree_interactions': episode_tree_interactions,
        'farming_cycles': episode_farming_cycles,
        'crop_diversity': episode_crop_diversity,
        'corn_harvested': episode_corn_harvested,
        'tomato_harvested': episode_tomato_harvested
    }
