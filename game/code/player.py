...
class Player(pygame.sprite.Sprite):
	def __init__(self, pos, group, collision_sprites, tree_sprites, interaction, soil_layer, toggle_shop):
		...
		self.env_events = None  # Reference to env_events list for RL reward tracking

		# sound (commented out for Lambda Cloud - no audio support)
		...
		if self.selected_tool == 'axe':
			for tree in self.tree_sprites.sprites():
				if tree.rect.collidepoint(self.target_pos):
					# Only damage if it's actually a Tree (has damage method)
					if hasattr(tree, 'damage'):
						tree.damage()
		
		if self.selected_tool == 'water':
			self.soil_layer.water(self.target_pos)
			# self.watering.play()  # Commented out for Lambda Cloud - no audio support
	...
	def use_seed(self):
		if self.seed_inventory[self.selected_seed] > 0:
			...
			# Emit invalid_action event when trying to plant with no seeds
			if self.env_events is not None:
				self.env_events.append('invalid_action')
