...
class Menu:
	def __init__(self, player, toggle_menu, env_events=None):

		# general setup
		self.player = player
		self.toggle_menu = toggle_menu
		self.env_events = env_events  # Reference to env_events list for RL reward tracking
			...
			if keys[pygame.K_SPACE]:
				...
				if self.index <= self.sell_border:
					if self.player.item_inventory[current_item] > 0:
						...
						# Emit sell_item event for RL rewards
						if self.env_events is not None:
							self.env_events.append('sell_item')
				...
				# buy
					...
					if self.player.money >= seed_price:
						...
						# Emit buy_seed event for RL rewards
						if self.env_events is not None:
							self.env_events.append('buy_seed')
